...
impl<'info> Sell<'info> {
    pub fn execute(&mut self, token_amount: u64, bumps: &SellBumps) -> Result<()> {
        require!(self.pool.enabled, DexiError::PoolDisabled);
        require!(token_amount > 0, DexiError::InvalidAmount);
        ...
        // Swap athlete tokens (X) in → USDC (Y) out
        let swap_result = curve
            .swap(LiquidityPair::X, token_amount, 1)
            .map_err(DexiError::from)?;

        let pool_seeds: &[&[&[u8]]] =
            &[&[POOL_SEED, self.pool.mint.as_ref(), &[bumps.pool]]];

        // Tokens flow in from user, USDC flows out from pool vault.
        self.transfer_user_to_pool_tokens(swap_result.deposit)?;
        self.transfer_pool_to_user_usdc(swap_result.withdraw, pool_seeds)?;

        Ok(())
    }

    fn transfer_user_to_pool_tokens(&self, amount: u64) -> Result<()> {
        transfer(
            CpiContext::new(
    ...
    fn transfer_pool_to_user_usdc(&self, amount: u64, seeds: &[&[&[u8]]]) -> Result<()> {
        transfer(
