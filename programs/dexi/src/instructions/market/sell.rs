...
impl<'info> Sell<'info> {
    pub fn execute(&mut self, token_amount: u64, bumps: &SellBumps) -> Result<()> {
        ...
        let swap_result = curve.swap(LiquidityPair::X, token_amount, 1)
            .map_err(DexiError::from)?;

        let pool_mint = self.pool.mint;
        let pool_seeds: &[&[&[u8]]] = &[&[POOL_SEED, pool_mint.as_ref(), &[bumps.pool]]];

        self.deposit_tokens(swap_result.deposit)?;
        self.withdraw_usdc(swap_result.withdraw, pool_seeds)?;
    ...
    pub fn withdraw_usdc(&self, amount: u64, seeds: &[&[&[u8]]]) -> Result<()> {
        let cpi_program = self.token_program.key();
        let cpi_accounts = Transfer {
